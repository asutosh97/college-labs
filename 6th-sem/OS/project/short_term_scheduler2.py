...
def MLFQ_base(n_queues, queue_details, processes, time0):
	...
	for idx in range(n_queues):
		...
		priority_counter[idx] = set_queue(ready_queue[idx], scheduling_algo_mapper(queue_details[idx]['scheduling_algo'])['criteria'], processes_set[idx], 0, timer, 0)
	...
	while processes_left:
		...
		for idx in range(n_queues):
			if not ready_queue[idx].empty():
				entered_for = True
				priority_counter[idx], timer_new, processes_left, gantt_chart = scheduling_algo_mapper(queue_details[idx]['scheduling_algo'])['step_function'](processes_set[idx], ready_queue[idx], result[idx], timer, processes_left, gantt_chart, queue_details[idx]['time_quantum'], priority_counter[idx])
				for step_idx in range(1, n_queues):
					next_idx = (idx + step_idx) % n_queues
					priority_counter[next_idx] = set_queue(ready_queue[next_idx], scheduling_algo_mapper(queue_details[next_idx]['scheduling_algo'])['criteria'], processes_set[next_idx], timer + 1, timer_new, priority_counter[next_idx])
				timer = timer_new
				break
		...
		if not entered_for:
			timer += 1
			for idx in range(n_queues):
				priority_counter[idx] = set_queue(ready_queue[idx], scheduling_algo_mapper(queue_details[idx]['scheduling_algo'])['criteria'], processes_set[idx], timer, timer, priority_counter[idx])

	def concat(a, b):
		return a + b

	return reduce(concat, result), gantt_chart, timer
